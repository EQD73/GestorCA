<?php

session_start();

if (!isset($_SESSION['codigo_usuario'])) {
    header("Location: ../index.php");
}

$nombre = $_SESSION['nombres'];
$codigo_rol = $_SESSION['codigo_rol'];
$tablam2 = $_SESSION['tablam2'];
$tablam3 = $_SESSION['tablam3'];
$codperiodo = $_SESSION['codigo_periodo'];
$nomperiodo = $_SESSION['nombre_periodo'];


include('conexion.php');
include('scriptsweet.php');

/* $codigoprograma     = $_REQUEST['CodigoPrograma'];
$nombreprograma     = $_REQUEST['NombreProg'];
$codigocurso        = $_REQUEST['CodigoCurso'];
$nombrecurso        = $_REQUEST['NombreCurso'];
$fechaconsigna     = date('d-m-Y');
$codigogrupo       = intval($_REQUEST['grupo']);
$semestre          = intval($_REQUEST['semestre']);
$codigodocente     = $_REQUEST['CodigoDocente'];
$nombredocente     = strval($_REQUEST['NomDocente']); */
$codigocurso        = $_REQUEST['CodigoCurso'];
$codigogrupo       = intval($_REQUEST['grupo']);
$codconsigna       = $_REQUEST['id_consigna'];
$resultados        = strval($_REQUEST['ResultadosTextarea1']);
$htts              = intval($_REQUEST['HorasTTS']);
$htps              = intval($_REQUEST['HorasTPS']);
$htis              = intval($_REQUEST['HorasTIS']);
/* variables semanas */
if (!empty($_POST['titulosem1'])) {
    $titulos1              = strval($_REQUEST['titulosem1']);
} else {
    $titulos1 = "";
}
if (!empty($_POST['rangoinicio1'])) {
    $rangoinicio1         = strval($_REQUEST['rangoinicio1']);
} else {
    $rangoinicio1 = "";
}
if (!empty($_POST['rangofinal1'])) {
    $rangofinal1         = strval($_REQUEST['rangofinal1']);
} else {
    $rangofinal1 = "";
} 

if (!empty($_POST['ContenidoSem1'])) {
    $contenidosem1         = strval($_REQUEST['ContenidoSem1']);
} else {
    $contenidosem1 = "";
}
if (!empty($_POST['NombreEstrategia1'])) {
    $nombreestrategia1    = strval($_REQUEST['NombreEstrategia1']);
} else {
    $nombreestrategia1 = "";
}
if (!empty($_POST['NombreMetodo1'])) {
    $nombremetodo1         = strval($_REQUEST['NombreMetodo1']);
} else {
    $nombremetodo1 = "";
}
//semana 2
if (!empty($_POST['titulosem2'])) {
    $titulos2              = strval($_REQUEST['titulosem2']);
} else {
    $titulos2 = "";
} 
if (!empty($_POST['rangoinicio2'])) {
    $rangoinicio2         = strval($_REQUEST['rangoinicio2']);
} else {
    $rangoinicio2 = "";
}
 if (!empty($_POST['rangofinal2'])) {
    $rangofinal2         = strval($_REQUEST['rangofinal2']);
} else {
    $rangofinal2 = "";
}
if (!empty($_POST['ContenidoSem2'])) {
    $contenidosem2         = strval($_REQUEST['ContenidoSem2']);
} else {
    $contenidosem2 = "";
}
if (!empty($_POST['NombreEstrategia2'])) {
    $nombreestrategia2    = strval($_REQUEST['NombreEstrategia2']);
} else {
    $nombreestrategia2 = "";
}
if (!empty($_POST['NombreMetodo2'])) {
    $nombremetodo2         = strval($_REQUEST['NombreMetodo2']);
} else {
    $nombremetodo2 = "";
}
//semana 3
if (!empty($_POST['titulosem3'])) {
    $titulos3              = strval($_REQUEST['titulosem3']);
} else {
    $titulos3 = "";
} 
if (!empty($_POST['rangoinicio3'])) {
    $rangoinicio3         = strval($_REQUEST['rangoinicio3']);
} else {
    $rangoinicio3 = "";
}
if (!empty($_POST['rangofinal3'])) {
    $rangofinal3         = strval($_REQUEST['rangofinal3']);
} else {
    $rangofinal3 = "";
}
if (!empty($_POST['ContenidoSem3'])) {
    $contenidosem3         = strval($_REQUEST['ContenidoSem3']);
} else {
    $contenidosem3 = "";
}
if (!empty($_POST['NombreEstrategia3'])) {
    $nombreestrategia3    = strval($_REQUEST['NombreEstrategia3']);
} else {
    $nombreestrategia3 = "";
}
if (!empty($_POST['NombreMetodo3'])) {
    $nombremetodo3         = strval($_REQUEST['NombreMetodo3']);
} else {
    $nombremetodo3 = "";
}
//semana 4
if (!empty($_POST['titulosem4'])) {
    $titulos4              = strval($_REQUEST['titulosem4']);
} else {
    $titulos4 = "";
}
if (!empty($_POST['rangoinicio4'])) {
    $rangoinicio4         = strval($_REQUEST['rangoinicio4']);
} else {
    $rangoinicio4 = "";
}
if (!empty($_POST['rangofinal4'])) {
    $rangofinal4         = strval($_REQUEST['rangofinal4']);
} else {
    $rangofinal4 = "";
}
if (!empty($_POST['ContenidoSem4'])) {
    $contenidosem4         = strval($_REQUEST['ContenidoSem4']);
} else {
    $contenidosem4 = "";
}
if (!empty($_POST['NombreEstrategia4'])) {
    $nombreestrategia4    = strval($_REQUEST['NombreEstrategia4']);
} else {
    $nombreestrategia4 = "";
}
if (!empty($_POST['NombreMetodo4'])) {
    $nombremetodo4         = strval($_REQUEST['NombreMetodo4']);
} else {
    $nombremetodo4 = "";
}
//semana 5
if (!empty($_POST['titulosem5'])) {
    $titulos5              = strval($_REQUEST['titulosem5']);
} else {
    $titulos5 = "";
}
if (!empty($_POST['rangoinicio5'])) {
    $rangoinicio5         = strval($_REQUEST['rangoinicio5']);
} else {
    $rangoinicio5 = "";
}
if (!empty($_POST['rangofinal9'])) {
    $rangofinal5         = strval($_REQUEST['rangofinal5']);
} else {
    $rangofinal5 = "";
}

if (!empty($_POST['ContenidoSem5'])) {
    $contenidosem5         = strval($_REQUEST['ContenidoSem5']);
} else {
    $contenidosem5 = "";
}
if (!empty($_POST['NombreEstrategia5'])) {
    $nombreestrategia5    = strval($_REQUEST['NombreEstrategia5']);
} else {
    $nombreestrategia5 = "";
}
if (!empty($_POST['NombreMetodo5'])) {
    $nombremetodo5         = strval($_REQUEST['NombreMetodo5']);
} else {
    $nombremetodo5 = "";
}
//semana 6
if (!empty($_POST['titulosem6'])) {
    $titulos6              = strval($_REQUEST['titulosem6']);
} else {
    $titulos6 = "";
}
if (!empty($_POST['rangoinicio6'])) {
    $rangoinicio6         = strval($_REQUEST['rangoinicio6']);
} else {
    $rangoinicio6 = "";
}
if (!empty($_POST['rangofinal6'])) {
    $rangofinal6         = strval($_REQUEST['rangofinal6']);
} else {
    $rangofinal6 = "";
}

if (!empty($_POST['ContenidoSem6'])) {
    $contenidosem6         = strval($_REQUEST['ContenidoSem6']);
} else {
    $contenidosem6 = "";
}
if (!empty($_POST['NombreEstrategia6'])) {
    $nombreestrategia6    = strval($_REQUEST['NombreEstrategia6']);
} else {
    $nombreestrategia6 = "";
}
if (!empty($_POST['NombreMetodo6'])) {
    $nombremetodo6         = strval($_REQUEST['NombreMetodo6']);
} else {
    $nombremetodo6 = "";
}
//semana 7
if (!empty($_POST['titulosem7'])) {
    $titulos7              = strval($_REQUEST['titulosem7']);
} else {
    $titulos7 = "";
}
if (!empty($_POST['rangoinicio7'])) {
    $rangoinicio7         = strval($_REQUEST['rangoinicio7']);
} else {
    $rangoinicio7 = "";
}
if (!empty($_POST['rangofinal7'])) {
    $rangofinal7         = strval($_REQUEST['rangofinal7']);
} else {
    $rangofinal7 = "";
}

if (!empty($_POST['ContenidoSem7'])) {
    $contenidosem7         = strval($_REQUEST['ContenidoSem7']);
} else {
    $contenidosem7 = "";
}
if (!empty($_POST['NombreEstrategia7'])) {
    $nombreestrategia7    = strval($_REQUEST['NombreEstrategia7']);
} else {
    $nombreestrategia7 = "";
}
if (!empty($_POST['NombreMetodo7'])) {
    $nombremetodo7         = strval($_REQUEST['NombreMetodo7']);
} else {
    $nombremetodo7 = "";
}
//semana 8
if (!empty($_POST['titulosem8'])) {
    $titulos8              = strval($_REQUEST['titulosem8']);
} else {
    $titulos8 = "";
}
if (!empty($_POST['rangoinicio8'])) {
    $rangoinicio8         = strval($_REQUEST['rangoinicio8']);
} else {
    $rangoinicio8 = "";
}
if (!empty($_POST['rangofinal8'])) {
    $rangofinal8         = strval($_REQUEST['rangofinal8']);
} else {
    $rangofinal8 = "";
}

if (!empty($_POST['ContenidoSem8'])) {
    $contenidosem8         = strval($_REQUEST['ContenidoSem8']);
} else {
    $contenidosem8 = "";
}
if (!empty($_POST['NombreEstrategia8'])) {
    $nombreestrategia8    = strval($_REQUEST['NombreEstrategia8']);
} else {
    $nombreestrategia8 = "";
}
if (!empty($_POST['NombreMetodo8'])) {
    $nombremetodo8         = strval($_REQUEST['NombreMetodo8']);
} else {
    $nombremetodo8 = "";
}
//semana 9
if (!empty($_POST['titulosem9'])) {
    $titulos9              = strval($_REQUEST['titulosem9']);
} else {
    $titulos9 = "";
}
if (!empty($_POST['rangoinicio9'])) {
    $rangoinicio9         = strval($_REQUEST['rangoinicio9']);
} else {
    $rangoinicio9 = "";
}
if (!empty($_POST['rangofinal9'])) {
    $rangofinal9         = strval($_REQUEST['rangofinal9']);
} else {
    $rangofinal9 = "";
}

if (!empty($_POST['ContenidoSem9'])) {
    $contenidosem9         = strval($_REQUEST['ContenidoSem9']);
} else {
    $contenidosem9 = "";
}
if (!empty($_POST['NombreEstrategia9'])) {
    $nombreestrategia9    = strval($_REQUEST['NombreEstrategia9']);
} else {
    $nombreestrategia9 = "";
}
if (!empty($_POST['NombreMetodo9'])) {
    $nombremetodo9         = strval($_REQUEST['NombreMetodo9']);
} else {
    $nombremetodo9 = "";
}
//semana 10
if (!empty($_POST['titulosem10'])) {
    $titulos10              = strval($_REQUEST['titulosem10']);
} else {
    $titulos10 = "";
}

if (!empty($_POST['rangoinicio10'])) {
    $rangoinicio10         = strval($_REQUEST['rangoinicio10']);
} else {
    $rangoinicio10 = "";
}
if (!empty($_POST['rangofinal10'])) {
    $rangofinal10         = strval($_REQUEST['rangofinal10']);
} else {
    $rangofinal10 = "";
}

if (!empty($_POST['ContenidoSem10'])) {
    $contenidosem10         = strval($_REQUEST['ContenidoSem10']);
} else {
    $contenidosem10 = "";
}
if (!empty($_POST['NombreEstrategia10'])) {
    $nombreestrategia10    = strval($_REQUEST['NombreEstrategia10']);
} else {
    $nombreestrategia10 = "";
}
if (!empty($_POST['NombreMetodo10'])) {
    $nombremetodo10         = strval($_REQUEST['NombreMetodo10']);
} else {
    $nombremetodo10 = "";
}
//semana 11
if (!empty($_POST['titulosem11'])) {
    $titulos11              = strval($_REQUEST['titulosem11']);
} else {
    $titulos11 = "";
}
if (!empty($_POST['rangoinicio11'])) {
    $rangoinicio11         = strval($_REQUEST['rangoinicio11']);
} else {
    $rangoinicio11 = "";
}
if (!empty($_POST['rangofinal11'])) {
    $rangofinal11         = strval($_REQUEST['rangofinal11']);
} else {
    $rangofinal11 = "";
}

if (!empty($_POST['ContenidoSem11'])) {
    $contenidosem11         = strval($_REQUEST['ContenidoSem11']);
} else {
    $contenidosem11 = "";
}
if (!empty($_POST['NombreEstrategia11'])) {
    $nombreestrategia11    = strval($_REQUEST['NombreEstrategia11']);
} else {
    $nombreestrategia11 = "";
}
if (!empty($_POST['NombreMetodo11'])) {
    $nombremetodo11         = strval($_REQUEST['NombreMetodo11']);
} else {
    $nombremetodo11 = "";
}
//semana 12
if (!empty($_POST['titulosem12'])) {
    $titulos12              = strval($_REQUEST['titulosem12']);
} else {
    $titulos12 = "";
}
if (!empty($_POST['rangoinicio12'])) {
    $rangoinicio12         = strval($_REQUEST['rangoinicio12']);
} else {
    $rangoinicio12 = "";
}
if (!empty($_POST['rangofinal12'])) {
    $rangofinal12         = strval($_REQUEST['rangofinal12']);
} else {
    $rangofinal12 = "";
}
if (!empty($_POST['ContenidoSem12'])) {
    $contenidosem12         = strval($_REQUEST['ContenidoSem12']);
} else {
    $contenidosem12 = "";
}
if (!empty($_POST['NombreEstrategia12'])) {
    $nombreestrategia12    = strval($_REQUEST['NombreEstrategia12']);
} else {
    $nombreestrategia12 = "";
}
if (!empty($_POST['NombreMetodo12'])) {
    $nombremetodo12         = strval($_REQUEST['NombreMetodo12']);
} else {
    $nombremetodo12 = "";
}
//semana 13
if (!empty($_POST['titulosem13'])) {
    $titulos13              = strval($_REQUEST['titulosem13']);
} else {
    $titulos13 = "";
}
if (!empty($_POST['rangoinicio13'])) {
    $rangoinicio13         = strval($_REQUEST['rangoinicio13']);
} else {
    $rangoinicio13 = "";
}
if (!empty($_POST['rangofinal13'])) {
    $rangofinal13         = strval($_REQUEST['rangofinal13']);
} else {
    $rangofinal13 = "";
}

if (!empty($_POST['ContenidoSem13'])) {
    $contenidosem13         = strval($_REQUEST['ContenidoSem13']);
} else {
    $contenidosem13 = "";
}
if (!empty($_POST['NombreEstrategia13'])) {
    $nombreestrategia13    = strval($_REQUEST['NombreEstrategia13']);
} else {
    $nombreestrategia13 = "";
}
if (!empty($_POST['NombreMetodo13'])) {
    $nombremetodo13         = strval($_REQUEST['NombreMetodo13']);
} else {
    $nombremetodo13 = "";
}
//semana 14
if (!empty($_POST['titulosem14'])) {
    $titulos14              = strval($_REQUEST['titulosem14']);
} else {
    $titulos14 = "";
}
if (!empty($_POST['rangoinicio14'])) {
    $rangoinicio14         = strval($_REQUEST['rangoinicio14']);
} else {
    $rangoinicio14 = "";
}
if (!empty($_POST['rangofinal14'])) {
    $rangofinal14         = strval($_REQUEST['rangofinal14']);
} else {
    $rangofinal14 = "";
}
if (!empty($_POST['ContenidoSem14'])) {
    $contenidosem14         = strval($_REQUEST['ContenidoSem14']);
} else {
    $contenidosem14 = "";
}
if (!empty($_POST['NombreEstrategia14'])) {
    $nombreestrategia14    = strval($_REQUEST['NombreEstrategia14']);
} else {
    $nombreestrategia14 = "";
}
if (!empty($_POST['NombreMetodo14'])) {
    $nombremetodo14         = strval($_REQUEST['NombreMetodo14']);
} else {
    $nombremetodo14 = "";
}
//semana 15
if (!empty($_POST['titulosem15'])) {
    $titulos15              = strval($_REQUEST['titulosem15']);
} else {
    $titulos15 = "";
}
if (!empty($_POST['rangoinicio15'])) {
    $rangoinicio15         = strval($_REQUEST['rangoinicio15']);
} else {
    $rangoinicio15 = "";
}
if (!empty($_POST['rangofinal15'])) {
    $rangofinal15         = strval($_REQUEST['rangofinal15']);
} else {
    $rangofinal15 = "";
}
if (!empty($_POST['ContenidoSem15'])) {
    $contenidosem15         = strval($_REQUEST['ContenidoSem15']);
} else {
    $contenidosem15 = "";
}
if (!empty($_POST['NombreEstrategia15'])) {
    $nombreestrategia15    = strval($_REQUEST['NombreEstrategia15']);
} else {
    $nombreestrategia15 = "";
}
if (!empty($_POST['NombreMetodo15'])) {
    $nombremetodo15         = strval($_REQUEST['NombreMetodo15']);
} else {
    $nombremetodo15 = "";
}
//semana 16
if (!empty($_POST['titulosem16'])) {
    $titulos16              = strval($_REQUEST['titulosem16']);
} else {
    $titulos16 = "";
}
if (!empty($_POST['rangoinicio16'])) {
    $rangoinicio16         = strval($_REQUEST['rangoinicio16']);
} else {
    $rangoinicio16 = "";
}
if (!empty($_POST['rangofinal16'])) {
    $rangofinal16         = strval($_REQUEST['rangofinal16']);
} else {
    $rangofinal16 = "";
}

if (!empty($_POST['ContenidoSem16'])) {
    $contenidosem16         = strval($_REQUEST['ContenidoSem16']);
} else {
    $contenidosem16 = "";
}
if (!empty($_POST['NombreEstrategia16'])) {
    $nombreestrategia16    = strval($_REQUEST['NombreEstrategia16']);
} else {
    $nombreestrategia16 = "";
}
if (!empty($_POST['NombreMetodo16'])) {
    $nombremetodo16         = strval($_REQUEST['NombreMetodo16']);
} else {
    $nombremetodo16 = "";
}
//semana 17
if (!empty($_POST['titulosem17'])) {
    $titulos17              = strval($_REQUEST['titulosem17']);
} else {
    $titulos17 = "";
}
if (!empty($_POST['rangoinicio17'])) {
    $rangoinicio17         = strval($_REQUEST['rangoinicio17']);
} else {
    $rangoinicio17 = "";
}
if (!empty($_POST['rangofinal17'])) {
    $rangofinal17         = strval($_REQUEST['rangofinal17']);
} else {
    $rangofinal17 = "";
}

if (!empty($_POST['ContenidoSem17'])) {
    $contenidosem17         = strval($_REQUEST['ContenidoSem17']);
} else {
    $contenidosem17 = "";
}
if (!empty($_POST['NombreEstrategia17'])) {
    $nombreestrategia17    = strval($_REQUEST['NombreEstrategia17']);
} else {
    $nombreestrategia17 = "";
}
if (!empty($_POST['NombreMetodo17'])) {
    $nombremetodo17         = strval($_REQUEST['NombreMetodo17']);
} else {
    $nombremetodo17 = "";
}
//semana 18
if (!empty($_POST['titulosem18'])) {
    $titulos18              = strval($_REQUEST['titulosem18']);
} else {
    $titulos18 = "";
}
if (!empty($_POST['rangoinicio18'])) {
    $rangoinicio18         = strval($_REQUEST['rangoinicio18']);
} else {
    $rangoinicio18 = "";
}
if (!empty($_POST['rangofinal18'])) {
    $rangofinal18         = strval($_REQUEST['rangofinal18']);
} else {
    $rangofinal18 = "";
}

if (!empty($_POST['ContenidoSem18'])) {
    $contenidosem18         = strval($_REQUEST['ContenidoSem18']);
} else {
    $contenidosem18 = "";
}
if (!empty($_POST['NombreEstrategia18'])) {
    $nombreestrategia18    = strval($_REQUEST['NombreEstrategia18']);
} else {
    $nombreestrategia18 = "";
}
if (!empty($_POST['NombreMetodo18'])) {
    $nombremetodo18         = strval($_REQUEST['NombreMetodo18']);
} else {
    $nombremetodo18 = "";
}
//semana1 postgrado
if (!empty($_POST['titulosem1p'])) {
    $titulos1p              = strval($_REQUEST['titulosem1p']);
} else {
    $titulos1p = "";
}

if (!empty($_POST['frangoinicio1'])) {
    $rangoinicio1p          = strval($_REQUEST['frangoinicio1']);
} else {
    $rangoinicio1p = "";
}
if (!empty($_POST['frangofinal1'])) {
    $rangofinal1p           = strval($_REQUEST['frangofinal1']);
} else {
    $rangofinal1p = "";
}
if (!empty($_POST['ContenidoSem1p'])) {
    $contenidosem1p         = strval($_REQUEST['ContenidoSem1p']);
} else {
    $contenidosem1p = "";
}
if (!empty($_POST['NombreEstrategia1p'])) {
    $nombreestrategia1p     = strval($_REQUEST['NombreEstrategia1p']);
} else {
    $nombreestrategia1p = "";
}
if (!empty($_POST['NombreMetodo1p'])) {
    $nombremetodo1p         = strval($_REQUEST['NombreMetodo1p']);
} else {
    $nombremetodo1p = "";
}
//semana2 postgrado
if (!empty($_POST['titulosem2p'])) {
    $titulos2p              = strval($_REQUEST['titulosem2p']);
} else {
    $titulos2p = "";
}
if (!empty($_POST['frangoinicio2'])) {
    $rangoinicio2p          = strval($_REQUEST['frangoinicio2']);
} else {
    $rangoinicio2p = "";
}
if (!empty($_POST['frangofinal2'])) {
    $rangofinal2p           = strval($_REQUEST['frangofinal2']);
} else {
    $rangofinal2p = "";
}
if (!empty($_POST['ContenidoSem2p'])) {
    $contenidosem2p         = strval($_REQUEST['ContenidoSem2p']);
} else {
    $contenidosem2p = "";
}
if (!empty($_POST['NombreEstrategia2p'])) {
    $nombreestrategia2p     = strval($_REQUEST['NombreEstrategia2p']);
} else {
    $nombreestrategia2p = "";
}
if (!empty($_POST['NombreMetodo2p'])) {
    $nombremetodo2p         = strval($_REQUEST['NombreMetodo2p']);
} else {
    $nombremetodo2p = "";
}
//semana3 postgrado
if (!empty($_POST['titulosem3p'])) {
    $titulos3p              = strval($_REQUEST['titulosem3p']);
} else {
    $titulos3p = "";
}
if (!empty($_POST['frangoinicio3'])) {
    $rangoinicio3p          = strval($_REQUEST['frangoinicio3']);
} else {
    $rangoinicio3p = "";
}
if (!empty($_POST['frangofinal3'])) {
    $rangofinal3p           = strval($_REQUEST['frangofinal3']);
} else {
    $rangofinal3p = "";
}
if (!empty($_POST['ContenidoSem3p'])) {
    $contenidosem3p         = strval($_REQUEST['ContenidoSem3p']);
} else {
    $contenidosem3p = "";
}
if (!empty($_POST['NombreEstrategia3p'])) {
    $nombreestrategia3p     = strval($_REQUEST['NombreEstrategia3p']);
} else {
    $nombreestrategia3p = "";
}
if (!empty($_POST['NombreMetodo3p'])) {
    $nombremetodo3p         = strval($_REQUEST['NombreMetodo3p']);
} else {
    $nombremetodo3p = "";
}
//semana4 postgrado
if (!empty($_POST['titulosem4p'])) {
    $titulos4p              = strval($_REQUEST['titulosem4p']);
} else {
    $titulos4p = "";
}
if (!empty($_POST['frangoinicio4'])) {
    $rangoinicio4p          = strval($_REQUEST['frangoinicio4']);
} else {
    $rangoinicio4p = "";
}
if (!empty($_POST['frangofinal4'])) {
    $rangofinal4p           = strval($_REQUEST['frangofinal4']);
} else {
    $rangofinal4p = "";
}
if (!empty($_POST['ContenidoSem4p'])) {
    $contenidosem4p         = strval($_REQUEST['ContenidoSem4p']);
} else {
    $contenidosem4p = "";
}
if (!empty($_POST['NombreEstrategia4p'])) {
    $nombreestrategia4p     = strval($_REQUEST['NombreEstrategia4p']);
} else {
    $nombreestrategia4p = "";
}
if (!empty($_POST['NombreMetodo4p'])) {
    $nombremetodo4p         = strval($_REQUEST['NombreMetodo4p']);
} else {
    $nombremetodo4p = "";
}
//semana5 postgrado
if (!empty($_POST['titulosem5p'])) {
    $titulos5p              = strval($_REQUEST['titulosem5p']);
} else {
    $titulos5p = "";
}
if (!empty($_POST['frangoinicio5'])) {
    $rangoinicio5p          = strval($_REQUEST['frangoinicio5']);
} else {
    $rangoinicio5p = "";
}
if (!empty($_POST['frangofinal5'])) {
    $rangofinal5p           = strval($_REQUEST['frangofinal5']);
} else {
    $rangofinal5p = "";
}
if (!empty($_POST['ContenidoSem5p'])) {
    $contenidosem5p         = strval($_REQUEST['ContenidoSem5p']);
} else {
    $contenidosem5p = "";
}
if (!empty($_POST['NombreEstrategia5p'])) {
    $nombreestrategia5p     = strval($_REQUEST['NombreEstrategia5p']);
} else {
    $nombreestrategia5p = "";
}
if (!empty($_POST['NombreMetodo5p'])) {
    $nombremetodo5p         = strval($_REQUEST['NombreMetodo5p']);
} else {
    $nombremetodo5p = "";
}
// validador
if (!empty($_POST['Validador1'])) {
    $validador1             = strval($_REQUEST['Validador1']);
} else {
    $validador1 = "";
}
if (!empty($_POST['Validador2'])) {
    $validador2             = strval($_REQUEST['Validador2']);
} else {
    $validador2 = "";
}
//$validador2             = strval($_REQUEST['Validador2']);

$QueryUpdate = ("UPDATE $tablam2
 SET     
    resultados_aprendizaje ='" . $resultados . "', 
	htts= '" . $htts . "',
	htps= '" . $htps . "',
	htis= '" . $htis . "',
    s1_titulo ='" . $titulos1 . "',
    s1_contenidos ='" . $contenidosem1 . "',
	s1_estrategia= '" . $nombreestrategia1 . "',
	s1_metodologia='" . $nombremetodo1 . "', 
	s2_titulo ='" . $titulos2 . "',
    s2_contenidos='" . $contenidosem2 . "',
	s2_estrategia='" . $nombreestrategia2 . "',
	s2_metodologia='" . $nombremetodo2 . "',
    s3_titulo ='" . $titulos3 . "',
	s3_contenidos='" . $contenidosem3 . "',
	s3_estrategia='" . $nombreestrategia3 . "',
	s3_metodologia='" . $nombremetodo3 . "',
	s4_titulo ='" . $titulos4 . "',
    s4_contenidos='" . $contenidosem4 . "',
	s4_estrategia='" . $nombreestrategia4 . "',
	s4_metodologia='" . $nombremetodo4 . "',
	s5_titulo ='" . $titulos5 . "',
    s5_contenidos='" . $contenidosem5 . "',
	s5_estrategia='" . $nombreestrategia5 . "',
	s5_metodologia='" . $nombremetodo5 . "',
	s6_titulo ='" . $titulos6 . "',
    s6_contenidos='" . $contenidosem6 . "',
	s6_estrategia='" . $nombreestrategia6 . "',
	s6_metodologia='" . $nombremetodo6 . "',
	s7_titulo ='" . $titulos7 . "',
    s7_contenidos='" . $contenidosem7 . "',
	s7_estrategia='" . $nombreestrategia7 . "',
	s7_metodologia='" . $nombremetodo7 . "',
	s8_titulo ='" . $titulos8 . "',
    s8_contenidos='" . $contenidosem8 . "',
	s8_estrategia='" . $nombreestrategia8 . "',
	s8_metodologia='" . $nombremetodo8 . "',
	s9_titulo ='" . $titulos9 . "',
    s9_contenidos='" . $contenidosem9 . "',
	s9_estrategia='" . $nombreestrategia9 . "',
	s9_metodologia='" . $nombremetodo9 . "',
	s10_titulo ='" . $titulos10 . "',
    s10_contenidos='" . $contenidosem10 . "',
	s10_estrategia='" . $nombreestrategia10 . "',
	s10_metodologia='" . $nombremetodo10 . "',
	s11_titulo ='" . $titulos11 . "',
    s11_contenidos=  '" . $contenidosem11 . "',
	s11_estrategia=  '" . $nombreestrategia11 . "',
	s11_metodologia='" . $nombremetodo11 . "',
	s12_titulo ='" . $titulos12 . "',
    s12_contenidos='" . $contenidosem12 . "',
	s12_estrategia='" . $nombreestrategia12 . "',
	s12_metodologia='" . $nombremetodo12 . "',
	s13_titulo ='" . $titulos13 . "',
    s13_contenidos='" . $contenidosem13 . "',
	s13_estrategia='" . $nombreestrategia13 . "',
	s13_metodologia='" . $nombremetodo13 . "',
	s14_titulo ='" . $titulos14 . "',
    s14_contenidos='" . $contenidosem14 . "',
	s14_estrategia='" . $nombreestrategia14 . "',
	s14_metodologia= '" . $nombremetodo14 . "',
	s15_titulo ='" . $titulos15 . "',
    s15_contenidos='" . $contenidosem15 . "',
	s15_estrategia='" . $nombreestrategia15 . "',
    s15_metodologia='" . $nombremetodo15 . "',
	s16_titulo ='" . $titulos16 . "',
    s16_contenidos='" . $contenidosem16 . "',
	s16_estrategia= '" . $nombreestrategia16 . "',
	s16_metodologia='" . $nombremetodo16 . "',
    s17_titulo ='" . $titulos17 . "',
    s17_contenidos='" . $contenidosem17 . "',
	s17_estrategia= '" . $nombreestrategia17 . "',
	s17_metodologia='" . $nombremetodo17 . "',
    s18_titulo ='" . $titulos18 . "',
    s18_contenidos='" . $contenidosem18 . "',
	s18_estrategia= '" . $nombreestrategia18 . "',
	s18_metodologia='" . $nombremetodo18 . "',
    s1_titulo_p ='" . $titulos1p . "',
    s1_rangoi_p ='" . $rangoinicio1p . "',
    s1_rangof_p ='" . $rangofinal1p . "',
	s1_contenidos_p='" . $contenidosem1p . "',
	s1_estrategia_p='" . $nombreestrategia1p . "',
	s1_metodologia_p='" . $nombremetodo1p . "',
    s2_titulo_p ='" . $titulos2p . "',
    s2_rangoi_p ='" . $rangoinicio2p . "',
    s2_rangof_p ='" . $rangofinal2p . "',
	s2_contenidos_p='" . $contenidosem2p . "',
	s2_estrategia_p='" . $nombreestrategia2p . "',
	s2_metodologia_p='" . $nombremetodo2p . "',
	s3_titulo_p ='" . $titulos3p . "',
    s3_rangoi_p ='" . $rangoinicio3p . "',
    s3_rangof_p ='" . $rangofinal3p . "',
    s3_contenidos_p='" . $contenidosem3p . "',
	s3_estrategia_p='" . $nombreestrategia3p . "',
	s3_metodologia_p='" . $nombremetodo3p . "',
    s4_titulo_p ='" . $titulos4p . "',
	s4_rangoi_p ='" . $rangoinicio4p . "',
    s4_rangof_p ='" . $rangofinal4p . "',
    s4_contenidos_p='" . $contenidosem4p . "',
	s4_estrategia_p='" . $nombreestrategia4p . "',
	s4_metodologia_p='" . $nombremetodo4p . "',
	s5_titulo_p ='" . $titulos5p . "',
    s5_rangoi_p ='" . $rangoinicio5p . "',
    s5_rangof_p ='" . $rangofinal5p . "',
    s5_contenidos_p= '" . $contenidosem5p . "',
	s5_estrategia_p= '" . $nombreestrategia5p . "',
	s5_metodologia_p= '" . $nombremetodo5p . "',
	validador1='" . $validador1 . "',
	validador2= '" . $validador2 . "'

WHERE id='" .$codconsigna. "' 
");

$updateConsigna = pg_query($conexion, $QueryUpdate);

//UPDATE tablam3 
$QueryUpdateR = ("UPDATE $tablam3
 SET 
    s1_titulo ='" . $titulos1 . "',
    s1_rangoi ='" . $rangoinicio1 . "',
    s1_rangof ='" . $rangofinal1 . "',
    s1_contenidos='" . $contenidosem1 . "',
    s2_titulo ='" . $titulos2 . "',
    s2_rangoi ='" . $rangoinicio2 . "',
    s2_rangof ='" . $rangofinal2 . "',
	s2_contenidos='" . $contenidosem2 . "',
    s3_titulo ='" . $titulos3 . "',
    s3_rangoi ='" . $rangoinicio3 . "',
    s3_rangof ='" . $rangofinal3 . "',
    s3_contenidos='" . $contenidosem3 . "',
    s4_titulo ='" . $titulos4 . "',
    s4_rangoi ='" . $rangoinicio4 . "',
    s4_rangof ='" . $rangofinal4 . "',
    s4_contenidos='" . $contenidosem4 . "',
    s5_titulo ='" . $titulos5 . "',
    s5_rangoi ='" . $rangoinicio5 . "',
    s5_rangof ='" . $rangofinal5 . "',
    s5_contenidos='" . $contenidosem5 . "',
    s6_titulo ='" . $titulos6 . "',
    s6_rangoi ='" . $rangoinicio6 . "',
    s6_rangof ='" . $rangofinal6 . "',
    s6_contenidos='" . $contenidosem6 . "',
    s7_titulo ='" . $titulos7 . "',
    s7_rangoi ='" . $rangoinicio7 . "',
    s7_rangof ='" . $rangofinal7 . "',
    s7_contenidos='" . $contenidosem7 . "',
    s8_titulo ='" . $titulos8 . "',
    s8_rangoi ='" . $rangoinicio8 . "',
    s8_rangof ='" . $rangofinal8 . "',
    s8_contenidos='" . $contenidosem8 . "',
    s9_titulo ='" . $titulos9 . "',
    s9_rangoi ='" . $rangoinicio9 . "',
    s9_rangof ='" . $rangofinal9 . "',
    s9_contenidos='" . $contenidosem9 . "',
    s10_titulo ='" . $titulos10 . "',
    s10_rangoi ='" . $rangoinicio10 . "',
    s10_rangof ='" . $rangofinal10 . "',
    s10_contenidos='" . $contenidosem10 . "',
    s11_titulo ='" . $titulos11 . "',
    s11_rangoi ='" . $rangoinicio11 . "',
    s11_rangof ='" . $rangofinal11 . "',
    s11_contenidos='" . $contenidosem11 . "',
    s12_titulo ='" . $titulos12 . "',
    s12_rangoi ='" . $rangoinicio12 . "',
    s12_rangof ='" . $rangofinal12 . "',
    s12_contenidos='" . $contenidosem12 . "',
    s13_titulo ='" . $titulos13 . "',
    s13_rangoi ='" . $rangoinicio13 . "',
    s13_rangof ='" . $rangofinal13 . "',
    s13_contenidos='" . $contenidosem13 . "',
    s14_titulo ='" . $titulos14 . "',
    s14_rangoi ='" . $rangoinicio14 . "',
    s14_rangof ='" . $rangofinal14 . "',
    s14_contenidos='" . $contenidosem14 . "',
    s15_titulo ='" . $titulos15 . "',
    s15_rangoi ='" . $rangoinicio15 . "',
    s15_rangof ='" . $rangofinal15 . "',
    s15_contenidos='" . $contenidosem15 . "',
    s16_titulo ='" . $titulos16 . "',
    s16_rangoi ='" . $rangoinicio16 . "',
    s16_rangof ='" . $rangofinal16 . "',
    s16_contenidos='" . $contenidosem16 . "',
    s17_titulo ='" . $titulos17 . "',
    s17_rangoi ='" . $rangoinicio17 . "',
    s17_rangof ='" . $rangofinal17 . "',
    s17_contenidos='" . $contenidosem17 . "',
    s18_titulo ='" . $titulos18 . "',
    s18_rangoi ='" . $rangoinicio18 . "',
    s18_rangof ='" . $rangofinal18 . "',
    s18_contenidos='" . $contenidosem18 . "',
    s1_titulo_p ='" . $titulos1p . "',
    s1_rangoi_p ='" . $rangoinicio1p . "',
    s1_rangof_p ='" . $rangofinal1p . "',
  	s1_contenidos_p='" . $contenidosem1p . "',
    s2_titulo_p ='" . $titulos2p . "',
    s2_rangoi_p ='" . $rangoinicio2p . "',
    s2_rangof_p ='" . $rangofinal2p . "',
    s2_contenidos_p='" . $contenidosem2p . "',
    s3_titulo_p ='" . $titulos3p . "',
    s3_rangoi_p ='" . $rangoinicio3p . "',
    s3_rangof_p ='" . $rangofinal3p . "',
    s3_contenidos_p='" . $contenidosem3p . "',
    s4_titulo_p ='" . $titulos4p . "',
    s4_rangoi_p ='" . $rangoinicio4p . "',
    s4_rangof_p ='" . $rangofinal4p . "',
   	s4_contenidos_p='" . $contenidosem4p . "',
    s5_titulo_p ='" . $titulos5p . "',
    s5_rangoi_p ='" . $rangoinicio5p . "',
    s5_rangof_p ='" . $rangofinal5p . "',
    s5_contenidos_p='" . $contenidosem5p . "'

WHERE codigo_asignatura='" .$codigocurso. "' AND grupo='" .$codigogrupo. "' AND codigo_periodo='" .$codperiodo. "' 
");

$updateRegistro = pg_query($conexion, $QueryUpdateR);

if ($updateConsigna && $updateRegistro){
    //echo "registro insertado";
    echo '<script>
    Swal.fire({
     icon: "success",
     title: "Exito...",
     text: "¡Registro Actualizado con Exito!",
     showConfirmButton: true,
     confirmButtonText: "Cerrar"
     }).then(function(result){
        if(result.value){                   
         window.location = "Consignador.php";
        }
     });
    </script>';
}else{
    
    echo '<script>
    Swal.fire({
     icon: "error",
     title: "Oops...",
     text: "¡Ocurrio un error, el registro no se pudo actualizar!",
     showConfirmButton: true,
     confirmButtonText: "Cerrar"
     }).then(function(result){
        if(result.value){                   
         window.location = "Consignador.php";
        }
     });
    </script>';
    
}

?>