        ...
        <form id="filtro-form" method='POST' class="row g-3">
            <div class="col-md-4">
                ...
                <select id="docente" name="docente" class="form-select">
                    ...
                    <?php
                    $stmt = $pdo->query("SELECT DISTINCT codigo_docente, nombre_docente FROM sistema.m1 WHERE codigo_docente IS NOT NULL AND TRIM(codigo_docente) != '' AND codigo_docente != '0' ORDER BY nombre_docente");
            ...
            <div class="col-md-4">
                ...
                <select id="periodo" name="periodo" class="form-select">
                    <option value="">Seleccione...</option>
                    <?php
                    $stmt = $pdo->query("SELECT DISTINCT codigo_periodo, nombre_periodo, descripcion FROM sistema.periodos WHERE estado= 'ACTIVO' ORDER BY codigo_periodo DESC");
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    <script>
        ...
        $(document).ready(function() {
            $("#exportExcel").click(function() {
                let docente = $("#docente").val();
                let periodo = $("#periodo").val();
                window.location.href = "export_excel_registro_g2.php?docente=" + docente + "&periodo=" + periodo; // Redirige a exportar_excel.php
            });
            ...
            $("#exportPDF").click(function() {
                ...
                $.ajax({
                    url: "guardar_grafico_registro2.php",
                    type: "POST",
                    data: {
                        image: imageData
                    },
                    success: function(response) {
                        if (response == "success") {
                            window.open("export_pdf_registro_g2.php?docente=" + docente + "&periodo=" + periodo, "_blank"); // Redirige a exportar_pdf.php; // Ejecuta la exportación a PDF después de guardar el gráfico
