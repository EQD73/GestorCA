 ...
 */
            ...
            const steps = [
              {
                ...
                status: "statusMain",
                percent: 30,
                message: "Actualizando tabla principal...",
              },
              {
                target: "progressM1",
                status: "statusM1",
                percent: 50,
                message: "Actualizando Microcurrículo (M1)...",
              },
              {
                target: "progressM2",
                status: "statusM2",
                percent: 70,
                message: "Actualizando Consignador Académico (M2)...",
              },
              {
                target: "progressM3",
                status: "statusM3",
                percent: 90,
                message: "Actualizando Registro de Actividades (M3)...",
              },
            ];
          ...
          .then((data) => {
            ...
            if (data.estado === "Éxito") {
              ...
              if (data.docente_cambiado) {
                $("#statusM1").html(
                  `Completado (${data.filas_afectadas.m1} filas)`
                );
                $("#statusM2").html(
                  `Completado (${data.filas_afectadas.m2} filas)`
                );
                $("#statusM3").html(
                  `Completado (${data.filas_afectadas.m3} filas)`
                );
                  ...
                  html: `
                                <div class="text-start">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <small class="text-muted">Tiempo de ejecución: ${
                                          data.detalle
                                        }</small>
                                    </div>
                                </div>
              ...
              Swal.fire({
                ...
                html: `<p>${data.mensaje}</p>
                               ${
                                 data.error_sql
                                   ? `<small class="text-muted">Detalle SQL: ${JSON.stringify(
                                       data.error_sql
                                     )}</small>`
                                   : ""
                               }`,
  ...
  $(
    "#codigo_docente, #codigo_asignatura, #codigo_programa, #codigo_periodo"
  ).on("change", function () {
...
function editar(d) {
  ...
  // Asignatura
  if (
    !$('#codigo_asignatura option[value="' + d.codigo_asignatura + '"]').length
  ) {
    $("#codigo_asignatura")
      .append(
        new Option(
